...
class DbSync:
    ...
    def handle_datatype_change(self, column_name, stream):
        ...
        for name, properties_schema in self.flatten_schema.items():
            ...
            if name.lower() == column_name.lower():
                ...
                self.logger.info(
                    f"{safe_column_name(altered_col)} {column_type(properties_schema)} "
                )
        ...
        update_altered_col_with_prev_data = "UPDATE  {} SET  {}={}".format(
            self.table_name(stream, is_stage=False), altered_col, column_name
        )
        ...
        self.logger.info(
            "Renaming versioned_col to original col :{}".format(
                rename_altered_to_original_col
            )
        )
    ...
    def update_columns(self):
        ...
        self.logger.info("Column dict %s", columns_dict)
        ...
        self.logger.info("Filtered dict -----########111 %s", filtered_column_dict)
        columns_to_replace = [
